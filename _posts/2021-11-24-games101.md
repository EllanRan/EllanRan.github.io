...
<br> 向量的加法 $$ \vec A + \vec B = \begin{bmatrix} x_a + x_b \\ y_a + y_b \\ z_a + z_b \end{bmatrix} $$
#### 向量的点积
$$ \vec A \cdot \vec B = x_ax_b + y_ay_b + z_az_b = \begin{bmatrix} x_a & y_a & z_a \end{bmatrix} \begin{bmatrix} x_b \\ y_b \\ z_b \end{bmatrix} = \vec A^T \vec B = ‖\vec A‖‖\vec B‖cosθ $$
